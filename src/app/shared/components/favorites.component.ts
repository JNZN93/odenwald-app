  ...
  template: `
    <div class="favorites-container">
      <div class="favorites-header">
        ...
        <p class="favorites-description">Artikel, die Sie häufig bestellen</p>
      </div>
...
export class FavoritesComponent implements OnInit, OnDestroy {
  ...
  loadFavorites() {
    ...
    this.favoritesService.getFavorites(this.restaurantId, this.limit)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
          this.favorites = response.favorites;
          this.hasMore = response.favorites.length === (this.limit || 10);
